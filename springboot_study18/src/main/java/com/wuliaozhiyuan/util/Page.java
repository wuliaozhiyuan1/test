...
public class Page {
	...
	public Page(){
		pd = Tools.getPageData();
		try {
			this.showCount = systemSetting.getPageCount();
		} catch (Exception e) {
			this.showCount = 15;
			//默认取100条数据
			charpicCount = 100;
	...
	public void setCharpicCount(int charpicCount) {
		this.charpicCount = charpicCount;
	}
	...
	public String getPageStrList() {
		StringBuffer sb = new StringBuffer();
		if(totalResult>0){
			sb.append("	<ul class=\"pagination pull-right no-margin\">\n");
			if(currentPage==1){
				sb.append("	<li><a>共<font color=red>"+totalResult+"</font>条</a></li>\n");
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage1\" style=\"width:60px;text-align:center;float:left\" placeholder=\"页码\"/></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\"  class=\"btn btn-mini btn-success\">跳转</a></li>\n");
				sb.append("	<li><a>首页</a></li>\n");
				sb.append("	<li><a>上页</a></li>\n");
			}else{
			...
			//跳转函数 
			sb.append("function toTZ(){");
			sb.append("var toPaggeVlue = document.getElementById(\"toGoPage1\").value;");
			sb.append("if(toPaggeVlue == ''){document.getElementById(\"toGoPage1\").value=1;return;}");
			sb.append("if(isNaN(Number(toPaggeVlue))){document.getElementById(\"toGoPage1\").value=1;return;}");
			sb.append("nextPage(toPaggeVlue);");
			sb.append("}\n");
			sb.append("</script>\n");
		}
		pageStrList = sb.toString();
		return pageStrList;
	}
	...
	public void setPageStrList(String pageStrList) {
		this.pageStrList = pageStrList;
	}
