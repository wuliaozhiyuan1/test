...
//				 }
//			 });
			 sitemailNum = Number(data.sitemailNum);
			 $("#sitemailNum").html(Number(sitemailNum));	//站内信未读总数
			 sitemailSound = data.sitemailSound;				//站内信提示音效
			 chatAddress = data.chatAddress;				//即时聊天服务器IP和端口
			 onLineAddress = data.onlineAddress;					//在线管理和站内信服务器IP和端口
			 online();									//连接在线
		}
	});
}
...
function online(){
	if (window.WebSocket) {
		websocket = new WebSocket(encodeURI('ws://'+onLineAddress)); 
...
function fhsms(){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = locat+"/sitemail/list.do?status='未读'";
	 diag.Width = 800;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
